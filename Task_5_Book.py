...
giant clams given to King François I by the Republic of Venice, and
...
wonderful genus Flabellina as well as the genus Oculina from Réunion
...
Tuamotu Islands. In still other localities, such as Réunion Island and
...
under the Count de La Pérouse had miscarried. I straightened suddenly.
...
In 1785 the Count de La Pérouse and his subordinate, Captain de
...
La Pérouse, ships whose disappearance had shaken the entire world. He
...
and ailing, set sail with the Count de La Pérouse. They headed to the
...
separated La Pérouse’s islands from the southeastern tip of Papua.
...
from the island of Ceylon to Sydney, also calling at King George Sound
and Melbourne.
...
Melbourne, Réunion Island, and Mauritius; far too much traffic for the
...
the Count de La Pérouse. For long hours they competed in speed with
...
escape the Provençal fishermen, who prize them as highly as did the
...
Frédol has noted. It’s poorly constructed, being “defective,” so to
...
Réunion Island, the barometer fell to 710 millimeters. At the close of
...
sentences = re.split(r"\.|\?|\!", Book) # rozdělilo test po větách
for index, sentens in enumerate(sentences):
        print(f"Je tam {index} vět")


worlds = re.split(r" ", Book) #rozdělilo text na slova
for index, worl in enumerate(worlds):
    print(f"Je tam {index} slov")

numbers = re.findall(r"[\d]+\,?[\d]+", Book) # Vypsalo čísla i s desetinou čárkou
for numb in numbers:
    print(numb)

years = re.findall(r"[\d]{4}", Book)
for year in years:
    print(year)

nautilus = re.findall(r"Nautilus", Book)
for index, nau in enumerate(nautilus):
    print(f"Slovo Nautilus je v textu {index} krát")

kilometers = re.findall(r"[\w]+ kilometers", Book)
for kil in kilometers:
    print(kil)

capitole = re.findall(r"CHAPTER", Book)
for index, cap in enumerate(capitole):
    print(f"{cap} je tam {index} krát.")

capitole_sent = re.findall(r"CHAPTER \d+", Book) #Nedokončeno
for cap in capitole_sent:
    print(cap)
